...
◦ capacity=<number> (default: 1) - Maximum drones that can occupy this
zone simultaneously
...
∗ capacity=<number> (default: 1) - Maximum drones that can
...
◦ Zone capacity constraints (capacity) and connection capacity (capacity).
...
• Zones with capacity=N metadata can contain up to N drones simultaneously.
...
• Connection capacity (capacity) defined on connections limits how many
...
• Any metadata block (e.g., [zone=... color=...] for zones, [capacity=...]
...
• Capacity values (capacity for zones, capacity for connections) must
be positive integers.
• The capacity capacity is ignored on the start_hub and end_hub zones: these
